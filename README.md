...
5. **分帳 / 註解**：在「查詢」頁展開收據，點品項的品名，跳出的小框可以打中文註解、
   按「分帳」選誰分幾個（含「公用支出」）；沒特別分的品項預設全員均分
...
- `split`：這個品項怎麼分。`[{ who, units }]`，`units` 是「分幾個」，
  ...
  **省略或空陣列 = 由 `people` 全員均分**（大部分品項都不用寫）
- `sharedBy`（舊格式）：還讀得懂，會當成每個人 `units: 1`
...
- 每個品項：`每個對象要付 = price × 該對象 units ÷ 全部 units`
- `split` 省略時就是 `price ÷ people 人數`，每人一份
- 標到「公用支出」的份額**不算在任何人頭上**，只會單獨列在「分帳結算 → 公用支出」
- 「分帳結算」頁**不算誰轉帳給誰**，而是依消費者分組：
  每個人列出自己吃到 / 用到的品項，其中不是自己付錢的用紅字（＝要還的錢），
  並依付款人小計「要還給誰、各多少」
